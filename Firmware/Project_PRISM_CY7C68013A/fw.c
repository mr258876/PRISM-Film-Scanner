...
// Registers which require a synchronization delay, see CY7C6801X technical reference section 15.14
...
WORD pWcidFeatureDscr;
WORD pWcidExtendedFeatureDscr;
WORD pStringDscr;
...
void main(void)
{
   ...
   pWcidFeatureDscr = (WORD)&WcidFeatureDscr;
   pWcidExtendedFeatureDscr = (WORD)&WcidExtendedFeatureDscr;
   ...
   if ((WORD)&DeviceDscr & 0xe000)
   {
      ...
      pWcidFeatureDscr -= offset;
      pWcidExtendedFeatureDscr -= offset;
      pStringDscr -= offset;
   }
...
void SetupCommand(void)
{
   ...
   case SC_GET_DESCRIPTOR: // *** Get Descriptor
      if (DR_GetDescriptor())
         ...
         case GD_STRING: // String
            // WCID 1.0 reads OS String at 0xEE, so we need some workarounds here.
            str_idx = SETUPDAT[2];
            if (str_idx == 0xEE)
            {
               str_idx = 4;   // OS String at index 4, Check dscr.a51
            }
            if (dscr_ptr = (void *)EZUSB_GetStringDscr(str_idx))
            {
   ...
   case SC_SET_FEATURE: // *** Set Feature
      ...
      break;
   case SC_GET_WCID_DESCRIPTOR: // *** Get WCID Descriptor (WCID 1.0)
      if (DR_GetWcidDescriptor())
         switch (SETUPDAT[4]) // Check wIndex low byte
         {
         case GWD_GET_FEATURE: // ** Get WCID feature descriptor
            SUDPTRH = MSB(pWcidFeatureDscr);
            SUDPTRL = LSB(pWcidFeatureDscr);
            break;
         case GWD_GET_EXTENDED_FEATURE: // ** Get WCID extended feature descriptor
            SUDPTRH = MSB(pWcidExtendedFeatureDscr);
            SUDPTRL = LSB(pWcidExtendedFeatureDscr);
            break;
         default:
            EZUSB_STALL_EP0(); // Stall End Point 0
         }
