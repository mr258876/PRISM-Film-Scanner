...
#define SC_GET_WCID_DESCRIPTOR 0x17 // Setup command: WCID Descriptor
...
#define GWD_GET_FEATURE 0x04          // Get WCID descriptor feature: Get Compatible ID Feature Descriptor
#define GWD_GET_EXTENDED_FEATURE 0x05 // Get WCID descriptor feature: Get Extended Properties Feature Descriptor
...
#define EZUSB_BP(addr)                         \
   BPADDRH = (BYTE)(((WORD)addr >> 8) & 0xff); \
   BPADDRL = (BYTE)addr
...
typedef struct
{
   DWORD length;        // WCID descriptor length
   BYTE spec_ver_minor; // Specification Version (BCD) minor
   BYTE spec_ver_major; // Specification Version (BCD) major
   WORD interfaces;     // Number of interfaces
} WCIDDSCR;
...
extern WORD pWcidFeatureDscr;
extern WORD pWcidExtendedFeatureDscr;
...
extern code WCIDDSCR WcidFeatureDscr;
extern code WCIDDSCR WcidExtendedFeatureDscr;
